...
/**
 * P5.js animated background component (Wrapper version)
...
const sketch: Sketch<MySketchProps> = (p) => {
  ...
  let interactionRef: RefObject<{ x: number; y: number }> | null = null;
  ...
  p.updateWithProps = (props) => {
    if (props.interactionRef) interactionRef = props.interactionRef;
    ...
    for (let i = 0; i < len; i++) {
      ...
      for (let j = i + 1; j < len; j++) {
        ...
        if (distSq < CONNECTION_DIST_SQ) {
          ...
          p.stroke(...config.connectionColor, lineAlpha);
  ...
  p.draw = () => {
    ...
    const mouseX = interactionRef?.current?.x ?? CONFIG.OFF_SCREEN;
    const mouseY = interactionRef?.current?.y ?? CONFIG.OFF_SCREEN;
